...
| [**14. Mantra, Japa & Sacred Verse Counter**](architecture/14-mantra-and-sacred-verse-counter.md) | Unified Mantra & Sacred Verse Counter (`mantra-japa-counter`), multi-tradition support (Gayatri, Maha Mrityunjaya, Aumkar, Ram Japa, Tasbih, Jesus Prayer, Universal), Hardware AEC & vocal resonance isolation, ambient background music safety margin, 3s ambient acoustic noise calibration with anchored floor baseline, 800Hz voice formant biquad filter, calibrated natural verse duration thresholds (4.0s / 1.2s pause), intra-verse pause bridging, autocorrelation pitch tracking, 108-bead Mala ring | `AcousticMantraSensorProvider`, `MantraCountManager`, `MantraCounterContent`, `MantraCounterSettingsSheet` |
| [**15. Voice-Driven Yoga / Physiotherapy Hold Timer**](architecture/15-voice-hold-timer-subsystem.md) | Hands-free voice-activated isometric hold & physiotherapy timer (`yoga-physio-hold-timer`), offline on-device speech recognizer, natural voice grammar parser, dual-cue TTS scheduler matching Pranayama Indian female timbre (`hi-IN-SwaraNeural`, pitch 1.18f, cadence 0.85f) with dynamic raised-cosine ducking, pre-session interactive configuration chips (Hold, Rest, Rounds, Cadence), unified `HoldTimerSettingsSheet` drawer with SharedPreferences persistence, circadian theme overhaul with Phosphor line icons, in-session parameter alterations, adaptive pacing controller, clinician safety limits, and CSV/JSON clinical session telemetry export | `HoldTimerEngine`, `HoldTimerManager`, `HoldTimerSessionLogger`, `VoiceHoldTimerParser`, `HoldTimerScreen`, `HoldTimerSettingsSheet` |
