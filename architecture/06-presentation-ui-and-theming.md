...
- **Punch-Hole Cutout Safe Geometry & Zen Button Grouping (`SessionScreen.kt`)**:
  ...
  - `SessionScreen` enforces `Modifier.displayCutoutPadding()` and implements a **decoupled header and footer architecture**:
    - **Header Left**: Breadcrumb session exit navigation (`ic_ph_back`) and contextual profile title.
    - **Header Right (Peripherals & Ambient Controls)**: Unifies external device controls (`Tv` Living Room & TV Casting Sheet, `VolumeUp` audio levels, and Sun/Moon `Theme` switcher). Redundant direct `CastButton` discovery has been consolidated entirely into `CastMirroringSheet` (Requirement E8), and duplicate top-right settings buttons have been removed.
    - **Footer Transport Controls**: Ergonomically clusters `Reset` (left), prominent primary `Play/Pause` (center), and `Settings` (`ic_ph_tune`, right) within comfortable single-hand thumb reach.
