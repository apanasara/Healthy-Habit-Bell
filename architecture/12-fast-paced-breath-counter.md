...
- **Ambient Acoustic Noise Floor Calibration (3-Second Silent Profiling Window)**:
  Before active stroke counting begins in `ACOUSTIC_MIC` mode, `BreathCountManager` executes a 3-second `PREPARATION` pause (90 frames $\times 32\text{ms} \approx 2.88\text{s}$) to measure stationary environmental noise (AC blower hum, ceiling fans, outdoor wind, rustling leaves, electronic mic noise floor).
  - **Pre-Session Preparation Integration (`startPreparationCalibration`)**: In standard sessions with preparation countdown enabled, this 3-second silent calibration executes during the final 3 seconds of pre-session preparation ($T=3\text{s} \dots 1\text{s}$), right after the initial *"Take your position"* prompt. At $T=0\text{s}$, the session transitions to `SessionStatus.RUNNING` and begins counting immediately at `00:00` without any secondary post-start calibration lag.
  - **Strict Acoustic Silence Law**: During this 3-second profiling window, all countdown chime strikes (Option C strikes 3, 2, 1) and spoken numeric cues (*"Three"*, *"Two"*, *"One"*) are strictly suppressed in `PreparationVoiceGuide`, preventing phone speaker self-feedback from corrupting the baseline noise floor.
